...
const ClockIcon: React.FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 mr-2 text-amber-300">
...
const GoogleMeetIcon: React.FC = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-2" viewBox="0 0 24 24">
    ...
    {/* Fix: Removed extraneous closing </g> tag which was causing a parsing error. */}
...
const ScheduleCard: React.FC<ScheduleCardProps> = ({ item }) => {
  return (
    <div className="bg-white/10 backdrop-blur-lg p-6 rounded-2xl shadow-xl border border-white/10 transition-all duration-300 hover:bg-white/20 hover:shadow-rose-500/10 hover:scale-[1.02]">
      <h3 className="text-xl font-bold text-stone-100 mb-3 font-serif">{item.title}</h3>
      <div className="flex items-center text-md text-stone-300 mb-4">
        <ClockIcon />
        <span>{item.time}</span>
      </div>
